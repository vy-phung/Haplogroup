...
def createName(name):
  ...
  #! ls /content/drive/MyDrive/OUCRUwork/RetrieveData/others/AccessNumber > list.txt
  for words in name.split(' '):
    ...
    if name.split(' ').index(words) == 0:
      ...
      if AccNum not in lis:
        ...
        # fix this command line
        #!file=accnum.txt;for id in `cat $file`; do ${HOME}/edirect/esummary -db nuccore -id $id -format medline | egrep "AUTHORS|TITLE|JOURNAL|LOCUS|PUBMED|isolate|ORGANISM" > /content/drive/MyDrive/OUCRUwork/RetrieveData/others/AccessNumber/$id.txt;done
      else:
